const vm = new Vue({
	...
	methods: {
		//跳转到编辑页面
		jumpLink: function(identify) {
			console.log(identify)
			window.location.href = '../../view/driver/componyDirverInformation.html?identify='+identify
		},
		//查询
		searchList: function() {
			var _this = this;
		...
		//邀请司机
		inviteDriver: function() {
			console.log(1111)
			window.location.href = 'invitingDriver.html'

		},
		getListData: function(index, tops) {
			var _this = this;
			var parmas = {
				likeString: _this.likeString,
				num: _this.num,
				size: _this.size
			}
			EasyAjax.ajax_Post_Json({
				url: 'driver/company/driver',
				...
				if(!res.success) {
					...
					if(_this.num == 1 && (collections == null || collections.length == 0)) { //第一页
						$('.driverList').append('<p class="noData" style="text-align:center;font-size:15px;padding:10px 0;color:#666;">暂无数据</p>');
						$('.weui-loadmore').hide();
						$('.allDataFinish').remove();
					} else if(_this.num != 1 && (collections == null || collections.length == 0)) { //最后一页
						_this.dLoading = false;
						$('.allDataFinish').remove();
						$('.driverList').append('<p class="allDataFinish" style="text-align:center;font-size:15px;padding:10px 0;color:#666;">已经到底了</p>');
						$('.weui-loadmore').hide();
					} else {
						$('.noData').remove();
