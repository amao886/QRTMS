var goodsDetailEdit = new Vue({
	...
	methods: {
		getGoodsListData: function() {
			...
			if(sessionStorage.getItem('orderPageStore')) {
				_this.order = JSON.parse(sessionStorage.getItem('orderPageStore')).order;
				_this.extra = JSON.parse(sessionStorage.getItem('orderPageStore')).extra;
				_this.commodities = JSON.parse(sessionStorage.getItem('orderPageStore')).commodities;
				_this.customDatas = JSON.parse(sessionStorage.getItem('orderPageStore')).customDatas;
		...
		submit: function() {
			...
			var roleSts = '';
			if(_this.type == 1) { // type 1:货主  2.承运商
				roleSts = 'shipper';
			...
			EasyAjax.ajax_Post_Json({
				url: 'enterprise/order/' + roleSts + '/edit/order',
