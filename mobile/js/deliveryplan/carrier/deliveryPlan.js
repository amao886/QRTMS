var planList = new Vue({
	...
	data: {
		...
		pageNum: 1, //当前页
		pageSize: 5, //每页传几条数据
		lastCompnayKey: '',//数据来源 0：上游分配， '':我方创建
		allocate: false, //是否分配 true：已分配， false：未分配
		ordinateState: 0, //接单状态 0:未接单，1：已接单 不传全部
	...
	methods: {
		getListData: function(index, tops) {
			...
			var baseValue = {
				lastCompnayKey: _this.lastCompnayKey,
			...
			if(_this.orderTakingState != 0) {
				baseValue["ordinateState"] = _this.ordinateState;
			}
			if(_this.orderTakingState == 2 && _this.subOrderTakingState != 0) {
				baseValue["subordinateState"] = _this.subordinateState;
			}
			EasyAjax.ajax_Post_Json({
				url: 'enterprise/plan/coveyer/search',
		...
		closeFilterView: function(ev) {
			...
			if(ev.target === $('.chooseCon')[0]) {
				...
				var filterData = sessionStorage.getItem("carrierFilterData");
				if(filterData) {
					var dataObj = JSON.parse(filterData);

					_this.searchText = dataObj.searchText;
					_this.dataSourceType = dataObj.dataSourceType;
					_this.deliverBillState = dataObj.deliverBillState;
					_this.orderTakingState = dataObj.orderTakingState;
				} else {
					_this.searchText = '';
					_this.dataSourceType = 0;
					_this.deliverBillState = 0;
					_this.orderTakingState = 0;
				}
			}
		},
		chooseDataSourceType: function(id) {
			var _this = this;

			_this.dataSourceType = id;

			if(id == 0) {
				_this.lastCompnayKey = '';
			} else if(id == 1) {
				_this.lastCompnayKey = id - 1;
			}
		},
		...
		chooseOrderTakingState: function(id) {
			var _this = this;

			_this.orderTakingState = id;

			_this.ordinateState = id - 1;

			if(_this.orderTakingState < 2) {
				_this.subOrderTakingState = 0;
				_this.subordinateState = 0;
			}
		},
		...
		confirm: function() {
			...
			var filterData = {
				...
				dataSourceType: _this.dataSourceType,
				deliverBillState: _this.deliverBillState,
				orderTakingState: _this.orderTakingState,
				subOrderTakingState: _this.subOrderTakingState
			}
			sessionStorage.setItem("carrierFilterData", JSON.stringify(filterData));
		...
		confirmOrderTaking: function(id) {
			var _this = this;
