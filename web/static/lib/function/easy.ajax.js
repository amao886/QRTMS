...
(function(window, $) {
		...
		$.ajax({
			...
			success: function(_resultData) {
				...
				if(_resultData.success == true || config.mustCallback) {
					...
					_common.toast(3, _resultData.message);
	...
	function _handleStatus(status) {
		switch(status) {
			case 404:
				_common.toast(3, '请求资源不存在：#' + status);
				break;
			case 400:
				_common.toast(3, '请求参数有误：#' + status);
				break;
			case 500:
				_common.toast(3, '服务器错误：#' + status);
				break;
			case 504:
				_common.toast(3, '链接超时：#' + status);
				break;
			default:
				_common.toast(3, '未知错误：#' + status);
				break;
		}
	}
