...
;(function($){
    $.fn.pgwSlideshow = function(options) {
        ...
        var setup = function() {
            ...
            pgwSlideshow.plugin.prepend('<div class="ps-current"><ul class="docs-pictures"></ul><span class="ps-caption"></span></div>');
            ...
            pgwSlideshow.plugin.find('.ps-list > ul > li').each(function() {
                ...
                if (element.image) {
                    currentElement.html('<img src="' + element.image + '" alt="' + (element.title ? element.title : '') + ' "><i class="qualified-big">合格</i><i class="unqualified-big">不合格</i>');
                } else if (element.thumbnail) {
                    currentElement.html('<img src="' + element.thumbnail + '" alt="' + (element.title ? element.title : '') + '" ><i class="qualified-big">合格</i><i class="unqualified-big">不合格</i>');
                }
        ...
        var fadeElement = function(element) {
            ...
            if (pgwSlideshow.currentSlide > 0) {
                ...
                currentElement.css('position', 'absolute').animate({
                    ...
                    currentElement.removeClass("imgselected");
                });
            }
            ...
            nextElement.css('position', 'absolute').show().animate({
                ...
                nextElement.addClass("imgselected");
        ...
        var displayElement = function(elementId, apiController, direction) {
            ...
            setTimeout(function() {
            	var createtime = $(".ps-selected").children("img").attr("createtime");
            	$("#receiptUploadTime").text(DateTimeToString(createtime));
            	var uname = $(".ps-selected").children("img").attr("uname");
            	$("#receiptUploadName").text(uname);
            	$('.ps-item').each(function(){
            		var status = $(this).children("img").attr("status");
                	if(status==1){
                		addbig("qualified",this);
                	}else if(status==0 && status!=""){
                		addbig("unqualified",this);
                	}
            	});
            	
            	var flag = $(".ps-selected").children(".selected").attr("flag");
            	if(flag==1){//合格
            		$('.imgselected').children('.unqualified-big').hide();
					$('.imgselected').children('.qualified-big').show();
            	}else if(flag==0){//不合格
            		$('.imgselected').children('.qualified-big').hide();
					$('.imgselected').children('.unqualified-big').show();
            	}
            }, 1000);
            return true;
        };
