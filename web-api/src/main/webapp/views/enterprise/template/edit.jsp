			...
			<template>
				<div class="d_addTop">
					<form action="post">
						...
						<input type="text" id="tempName" v-model="template.name" />

						<span style="margin-left: 30px;"></span>
						<label for="mergeType">导入数据合并方式</label>
						<input type="radio" name="mergeType" v-model="template.mergeType" value="0" style="margin-left: 10px; cursor: pointer;">不合并
						<input type="radio" name="mergeType" v-model="template.mergeType" value="1" style="margin-left: 10px; cursor: pointer;">{{category==1? '按送货单号合并' : '按发货计划单号合并'}}
						<input type="radio" name="mergeType" v-model="template.mergeType" value="2" style="margin-left: 10px; cursor: pointer;">按收货信息合并
					</form>
				</div>
				<div class="d_addCenter" style="height: 250px;">
					<div class="setTit">设置结果预览：（拖拽字段可调整前后顺序）</div>
					<draggable :list="selects" :options="{touchStartThreshold:2000}">
				...
				<div class="d_addBottom">
							...
							<div class="col-lg-4">
								...
								<table border="1" class="stepTab">
									...
									<tr v-for="(b, index) in alls" v-if="b.type==1">
										...
										<td><label class="chooseFiledCheckBox"><input type="checkbox" v-model="b.required"><span style="cursor: pointer;"></span></label></td>
									</tr>
								</table>
								</div>
							</div>
								...
								<div class="addstep fl">
									...
									<table border="1" class="stepTab">
										...
										<tr v-for="(b, index) in alls" v-if="b.type==2">
											<td>{{b.name}}</td>
											<td>{{b.description}}</td>
											<td><label class="chooseFiledCheckBox"><input type="checkbox" v-model="b.checked" @change="selectOptional(b.index)"><span></span></label></td>
											<td><label class="chooseFiledCheckBox"><input type="checkbox" v-model="b.required" :disabled="!b.checked"><span></span></label></td>
										</tr>
									</table>
								</div>
							</div>
							<div class="col-lg-4">
								...
								<table border="1" class="stepTab customTab">
									...
									<tr v-for="(c, index) in customs">
										<td><input type="text" v-model="c.name"></td>
										...
										<td><label class="chooseFiledCheckBox"><input type="checkbox" v-model="c.checked" :disabled="c.name == null || c.name ==''" @change="selectCustom(c.index)"><span></span></label></td>
										<td><label class="chooseFiledCheckBox"><input type="checkbox" v-model="c.required" :disabled="(c.name == null || c.name =='') || !c.checked"><span></span></label></td>
									</tr>
								</table>
								</div>
                                ...
                                <div class="addstep fl">
                                    <div class="addstep fl" style="width: 250px;">
                                        ...
                                        <div style="margin-left: 20px;">
                                            插入<input type="number" min="0" max="20" v-model="emptyCount" class="emptyNumber">列空白列
                                        </div>
					...
					<div class="dabBottom  fixed-element">
		...
		$(document).ready(function() {
			...
			require(['vue', 'vuedraggable'], function(Vue, draggable) {
				...
				var v = new Vue({
                    ...
                    computed: {
                        customs: function () {
                            var ssss = this.alls.filter(function(o) {
                                return o.type === 3;
                            });
                            console.log(ssss);
                            return ssss;
					...
					methods: {
						loadTemplate: function() {
							...
							$.util.json(base_url + "/enterprise/template/detail", { templateKey: vm.templateKey }, function(data) {
								if(data.success) {
									...
									if(data.template) {
                                        vm.category = data.template.category
						...
						selectCustom: function(index) {
							var _this = this;
                            _this.alls.filter(function(o) {
                                return o.index === index;
                            }).forEach(function(obj){
                                if(obj.checked) {
                                    _this.selects.push(obj);
                                } else {
                                    obj.required = false;
                                    _this.selects = _this.selects.filter(function(o) {
                                        return o.index !== index;
                                    });
                                }
                            });
						...
						changeCustom: function(index) {
							if(index >= 0) {
							    ...
							    this.alls.push({ type: 3, name: '', description: '', index: this.lastIndex, required:false, checked:false });
							}
						},
