            ...
            <div class="clearfix">
                <div class="fl col-min-new radio-wrapper">
                    ...
                    <div class="radio-item">
                        ...
                        <input type="radio" class="receiptFettle_search" name="fettle" id="scan2" value="2">
                    </div>
                    <div class="radio-item">
    ...
    <div class="table-style" id="list-manage">
        <template>
            <div class="handle-box">
                <button class="layui-btn layui-btn-normal" v-on:click="imports">导入发货单</button>
            ...
            <table>
                ...
                <tr v-for="o in collection" v-if="collection.length > 0">
                    <td><input type="checkbox" name="orderId" v-bind:value="o.id" v-bind:deliveryNo="o.deliveryNo"/></td>
                    <td>
               ...
               <%--     <td>{{o.orderNo}}</td>--%>
                    ...
                    <td>
                        ...
                        <a class="linkBtn" v-if="o.receiptFettle == 0 && o.fettle < 4" @click="edit(o.id)">编辑</a>
                    </td>
                </tr>
                </tbody>
            </table>
...
<div class="add_enterprise_panel" style="display: none">
    <div class="model-base-box add_enterprise" style="width: 480px;overflow: hidden;">
        <form action="${basePath}/enterprise/order/shipper/import/excel" enctype="multipart/form-data" method="post">
    ...
    $(document).ready(function () {
        ...
        function importByExcel(element) {
            ...
            panel.addButton(function () {
                ...
                $.ajax({
                    url: base_url + "/enterprise/order/shipper/import/excel",
                    type: 'POST',
                    data: formData,
        ...
        function addEnterprise(){
            var panel = $.util.panel('添加客户', $(".add_enterprise_panel").html());
            panel.addButton(function () {
                var editBody = this.$body;
                var parmas = {
                    "companyName": editBody.find("input[name='companyName']").val(),
                    "legalName": "法人姓名",
                    "codeOrg": "组织机构代码"
                };
                $.util.json(base_url + "/special/backstage/company/save", parmas, function(data){
                    if (data.success) {
        ...
        function deletes(element) {
            ...
            $.util.confirm("提示", "确定要删除吗?", function(){
                $.util.json(base_url + '/enterprise/order/shipper/manage/delete', parmas, function (data) {
            ...
            methods: {
                ...
                edit: function (id) {//编辑
                    layer.full(layer.open({
                        type: 2,
                        title:'编辑订单信息',
                        content: base_url + "/enterprise/order/manage/edit?type=1&orderKey="+ id,
                        end: function () {
                            searchManage();
                        }
                    }));
        ...
        var searchManage = function (num) {
            ...
            $.util.json(base_url + '/enterprise/order/shipper/manage/search', parmas, function (data) {
                if (data.success) {//处理返回结果
