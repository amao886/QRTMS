...
<div id="order-entrance-wrapper">
    ...
    <div>
        <!-- Form -->
        <el-dialog title="选择模板" :visible.sync="dialogFormVisible" :show-close="false" :close-on-press-escape="false" :close-on-click-modal="false"
                   style="width: 1200px">
            <el-form :model="board">
                <el-form-item label="请选择模板" :label-width="formLabelWidth">
                    <el-select v-model="board.region" placeholder="请选择模板">
                        <el-option v-for="template in templates" :value="template.key" :label="template.name"></el-option>
                    </el-select>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="showToggle">确 定</el-button>
            </div>
        </el-dialog>
    </div>
    <!--        ------------------------------------    v-show="isshow"     -->
            ...
            <div>
                <el-form-item label=" 发货地址：" v-if="required.distributeAddress != null">
                    <select id="province3" @change="loadCity(4, 1)" ref='CityId3' v-model="areaSelects.p4">
                        ...
                        <option v-for="item in areas.p4" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
                    <!--  -->
                    <select id="city3" @change="loadCity(4, 2)" ref="CountyId3" v-model="areaSelects.c4">
                        <option value="">--请选城市--</option>
                        <option v-for="item in areas.c4" :key="item.id" :label="item.name" :value="item.id">
                        </option>
                    </select>

                    <select id="county3" v-model='areaSelects.d4' @change="loadCity(4, 3)">
                        <option value="">--请选区县--</option>
                        <option v-if="areas.d4 != null" v-for="item in areas.d4" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
            ...
            <div>
                <el-form-item label=" 始发地：" v-if="required.originStation != null">
                    ...
                    <select id="province1" @change="loadCity(2, 1)" ref='CityId1' v-model="areaSelects.p2">
                        <option value="">--请选省份--</option>
                        <option v-for="item in areas.p2" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
                    ...
                    <select id="city1" @change="loadCity(2, 2)" ref="CountyId1" v-model="areaSelects.c2">
                        <option value="">--请选城市--</option>
                        <option v-for="item in areas.c2" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
                    <!-- 区县 -->
                    <select id="county1" v-model='areaSelects.d2' @change="loadCity(2, 3)">
                        <option value="">--请选区县--</option>
                        <option v-if="areas.d2 != null" v-for="item in areas.d2" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
                </el-form-item>
            </div>
            ...
            <div>
                <el-form-item label=" 收货地址：" v-if="required.receiveAddress != null">
                    ...
                    <select id="province" @change="loadCity(1, 1)" ref='CityId' v-model="areaSelects.p1">
                        <option value="">--请选省份--</option>
                        <option v-for="item in areas.p1" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
                    ...
                    <select id="city" @change="loadCity(1, 2)" ref="CountyId" v-model="areaSelects.c1">
                        <option value="">--请选城市--</option>
                        <option v-for="item in areas.c1" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
                    <!-- 区县 -->
                    <select id="county" ref="county" v-model='areaSelects.d1' @change="loadCity(1, 3)">
                        ...
                        <option v-for="item in areas.d1" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
            ...
            <div>
                <el-form-item label=" 目的地：" v-if="required.arrivalStation != null">
                    ...
                    <select id="province2" @change="loadCity(3, 1)" ref='CityId2' v-model="areaSelects.p3">
                        <option value="">--请选省份--</option>
                        <option v-for="item in areas.p3" :key="item.id" :label=" item.name" :value=" item.id ">
                        </option>
                    </select>
                    ...
                    <select id="city2" @change="loadCity(3, 2)" ref="CountyId2" v-model="areaSelects.c3">
                        <option value="">--请选城市--</option>
                        <option v-for="item in areas.c3" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
                    <!-- 区县 -->
                    <select id="county2" v-model='areaSelects.d3' @change="loadCity(3, 3)">
                        <option value="">--请选区县--</option>
                        <option v-if="areas.d3 != null" v-for="item in areas.d3" :key="item.id" :label=" item.name" :value="item.id">
                        </option>
                    </select>
                </el-form-item>
            </div>
            ...
            methods: {
                ...
                setArea:function(i, pn, cn, dn){
                    ...
                    _this.loadCity(i, 1, function(){
                        if(!cn){ return; }
                        var city = _this.indexOfName(_this.areas['c' + i] , cn);
                ...
                validate:function(key, value){
                    ...
                    // else if("required.deliveryNo" != null){
                    //     return false;
                    // }
                    // if('shipperName' == key && !value){
                    //     _this.$message.error('发货方不能为空');
                    //     return false;
                    // }
                    return true;
                },
                saveForm: function () {
                    ...
                    $.each(_this.form, function(i, item){
                        ...
                        var _key = _this.required[i];
                        if(_key){
                            //1:收货地址 2:始发地 3:目的地 4:发货地址
                            if('receiveAddress' == i){
                                waybillInfo[_key] = $.trim(_this.joinArea(1)) + $.trim(item);
                            }else if('originStation' == i){
                                waybillInfo[_key] = $.trim(_this.joinArea(2)) + $.trim(item);
                            }else if('arrivalStation' == i){
                                waybillInfo[_key] = $.trim(_this.joinArea(3)) + $.trim(item);
                            }else if('distributeAddress' == i){
                                waybillInfo[_key] = $.trim(_this.joinArea(4)) + $.trim(item);
                            }else{
                                waybillInfo[_key] = $.trim(item);
                            }
                        }
                    });
                    $.each(_this.list, function(index, item) {
                        ...
                        $.each(item, function (j, v) {
                            var _key = _this.required[j];
                            if(_key){
                                commodity[_key] = $.trim(v);
                            }
                        })
                    ...
                    $.each(_this.optional, function(i, v){
                        waybillInfo[v[1]] = $('#'+ v[1]).val();
                    });
                    $.util.json(base_url + "/enterprise/template/convey/entrance", {
                        templateKey: _this.board.region,
                        waybillInfo: JSON.stringify(waybillInfo),
                        commodities: JSON.stringify(commodities)
                    }, function (data) {
                        if (data.success) {
                           window.location.href= base_url+'/enterprise/order/traffic/conveyer'//保存页面跳转
                        }else{
                ...
                showToggle: function () {
                    if (this.board.region == "") {
                        this.$message.error('请选择模板');
                        return;
                    }
                    var _this = this;
                    _this.dialogFormVisible = false;
                    $.util.json(base_url + "/enterprise/template/templateInfo/" + _this.board.region, null, function (data) {
                        if (data.success) {
